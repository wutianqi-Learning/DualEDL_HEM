    ...
    def _save_best_checkpoint(self, runner, metrics) -> None:
        ...
        for key_indicator, rule in zip(self.key_indicators, self.rules):
            ...
            for key, val in metrics.items():
                last_metrics[f'{key}'] = val
            runner.message_hub.update_info('last_metrics', last_metrics)
            ...
            key_indicator_filename = key_indicator.replace(' ', '_').replace('(', '').replace(')', '')
            best_ckpt_name = f'best_{key_indicator_filename}_{best_score_str}_{ckpt_filename}'
            if len(self.key_indicators) == 1:
