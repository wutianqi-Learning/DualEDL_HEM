...
class EvidenceWeightLoss2(nn.Module):
    def __init__(self,
                 loss_weight=1.0,
                 eps=1e-10,
                 class_num=9,
        ...
        """
        super(EvidenceWeightLoss2, self).__init__()
    ...
    def forward(self,
        ...
        weight_vd = calculate_weighted_vd(alpha.detach(), p)
        weight_vd_seg = weight_vd.view(-1, 1)
        ...
        # weight_kl_seg = torch.exp(torch.sum(label * (torch.log(S) - torch.log(alpha)), dim=1, keepdim=True))
        
        # dis_mask = kwards['dis_logit'] + 1
        # weight_vd = calculate_weighted_vd(dis_mask.detach(), p)
        weight_vd_dis = weight_vd.view(-1, 1)
        ...
        # dis_mask_S = torch.sum(dis_mask, dim=1, keepdim=True)
        
        # weight_kl_dis = torch.exp(torch.sum(label * (torch.log(dis_mask_S.detach()) - torch.log(dis_mask.detach())), dim=1, keepdim=True))
        # L_ace_kl = torch.sum(label * (torch.digamma(S) - torch.digamma(alpha)), dim=1, keepdim=True) * weight_kl_dis * ramps
        # loss_ml = torch.sum(label * (torch.log(S) - torch.log(alpha)), dim=1, keepdim=True)
        L_ace = torch.sum(label * (torch.digamma(S) - torch.digamma(alpha)), dim=1, keepdim=True) * (weight_vd_seg + 1)
        # L_dice = Weight_TDice(evidence, soft_p, criterion_dl, weight_vd_seg)
