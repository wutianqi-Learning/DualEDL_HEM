...
class UNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.model(x)
        evidence = softplus_evidence(x)
        # e = torch.exp(torch.tanh(evidence))
        return evidence
        # return x
...
class UNet_sdf(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[Any, Tensor]:
        x = self.model(x)
        evidence = softplus_evidence(x)
        # dis = self.conv_tanh(x)
        # evidence_axu = softplus_evidence(self.conv1331(x))
        evidence_axu = relu_evidence(x)
        # e = torch.exp(torch.tanh(evidence))
        return evidence, evidence_axu
        # return x
