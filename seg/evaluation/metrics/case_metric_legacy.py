...
def calculate_metric_percase(cls_index, pred, gt, metrics):
    ...
    for metric in metrics:
        ret = ALL_METRICS[metric](pred, gt, confusion_matrix, nan_for_nonexisting=False)
        rets.append(ret)
    return rets
...
class CaseMetric(BaseMetric):
    ...
    def __init__(self,
                 ...
                 metrics: List[str] = ['Dice', 'Jaccard', 'HD95'],
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        total_pred_label = np.stack([results[i]['pred'][0] for i in range(len(results))], axis=0)
        total_label = np.stack([results[i]['label'][0] for i in range(len(results))], axis=0)
